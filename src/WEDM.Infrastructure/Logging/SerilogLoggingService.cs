...
public sealed class SerilogLoggingService : ILoggingService, IDisposable
{
    ...
    private static string GenerateHtmlReport(DeploymentReport report)
    {
        ...
        // ── Process lifecycle ──────────────────────────────────────────────────
        AppendProcessLifecycleSection(sb, report);
    ...
    private static void AppendProcessLifecycleSection(StringBuilder sb, DeploymentReport report)
    {
        var lc = report.ProcessLifecycle;
        if (lc is null) return;

        var allCleanColor = lc.AllCleaned ? "#3FB950" : "#D29922";
        var heading = lc.AllCleaned
            ? "Process Lifecycle — Clean"
            : "Process Lifecycle — Residuals Detected";

        sb.AppendLine($"<div class='card' style='border-color:{allCleanColor}60'>");
        sb.AppendLine($"<h2 style='color:{allCleanColor}'>{heading}</h2>");

        // Summary stats
        sb.AppendLine("<div class='grid-2' style='margin-bottom:16px'>");
        KvRowStatic(sb, "Detected",           lc.DetectedProcesses.Count.ToString());
        KvRowStatic(sb, "Terminated",         lc.TotalTerminated.ToString());
        KvRowStatic(sb, "Still running",      lc.StillRunning.Count.ToString());
        KvRowStatic(sb, "Orphans found",      lc.OrphansFound.Count.ToString());
        KvRowStatic(sb, "Orphans reconciled", lc.OrphansReconciled.Count.ToString());
        KvRowStatic(sb, "External (skipped)", lc.ExternalOrphans.Count.ToString());
        KvRowStatic(sb, "Failed to stop",     lc.TotalFailed.ToString());
        KvRowStatic(sb, "All cleaned",        lc.AllCleaned ? "Yes ✔" : "No ⚠");
        sb.AppendLine("</div>");

        // Still-running warnings
        if (lc.StillRunning.Count > 0)
        {
            sb.AppendLine("<p style='color:#F85149;font-size:.88rem;margin-bottom:12px'>" +
                "⚠ The following Oracle processes were still running at report generation time. " +
                "Rollback or filesystem cleanup may be incomplete.</p>");
        }

        // Termination results table
        if (lc.TerminationResults.Count > 0)
        {
            sb.AppendLine("<table style='margin-bottom:14px'>");
            sb.AppendLine("<tr><th>PID</th><th>Process</th><th>Kind</th><th>Stage</th><th>Duration</th><th>Error</th></tr>");
            foreach (var r in lc.TerminationResults)
            {
                var (cls, icon) = r.Stage switch
                {
                    Domain.Models.TerminationStage.AlreadyExited => ("ok",   "✔"),
                    Domain.Models.TerminationStage.Graceful       => ("ok",   "✔"),
                    Domain.Models.TerminationStage.Escalated      => ("warn", "⬆"),
                    Domain.Models.TerminationStage.ForcedKill     => ("warn", "💀"),
                    Domain.Models.TerminationStage.Failed         => ("fail", "✘"),
                    Domain.Models.TerminationStage.Skipped        => ("skip", "⊘"),
                    _                                              => ("skip", "?")
                };
                var error = string.IsNullOrWhiteSpace(r.Error) ? "—" : r.Error;
                sb.AppendLine($"<tr>" +
                    $"<td style='font-variant-numeric:tabular-nums'>{r.ProcessId}</td>" +
                    $"<td>{HtmlEncode(r.ProcessName)}</td>" +
                    $"<td style='color:#8B949E;font-size:.82rem'>{r.Kind}</td>" +
                    $"<td class='{cls}'>{icon} {r.Stage}</td>" +
                    $"<td style='white-space:nowrap'>{r.Duration.TotalSeconds:F1}s</td>" +
                    $"<td style='color:#8B949E;font-size:.82rem'>{HtmlEncode(error)}</td>" +
                    $"</tr>");
            }
            sb.AppendLine("</table>");
        }
        else if (lc.DetectedProcesses.Count == 0)
        {
            sb.AppendLine("<p class='section-empty'>No Oracle processes were active during this deployment session.</p>");
        }

        // External orphans (never terminated)
        if (lc.ExternalOrphans.Count > 0)
        {
            sb.AppendLine("<p style='color:#D29922;font-size:.88rem;margin:12px 0 6px'>" +
                "⚠ External Oracle processes were detected but NOT terminated (external-process protection):</p>");
            sb.AppendLine("<table><tr><th>PID</th><th>Process</th><th>Kind</th><th>Reason</th></tr>");
            foreach (var ext in lc.ExternalOrphans)
            {
                sb.AppendLine($"<tr>" +
                    $"<td>{ext.ProcessId}</td>" +
                    $"<td>{HtmlEncode(ext.ProcessName)}</td>" +
                    $"<td style='color:#8B949E'>{ext.Kind}</td>" +
                    $"<td style='color:#8B949E;font-size:.82rem'>{HtmlEncode(ext.ClassificationReason)}</td>" +
                    $"</tr>");
            }
            sb.AppendLine("</table>");
        }
        ...
        // Manual action items
        if (lc.ManualActionItems.Count > 0)
        {
            sb.AppendLine("<div style='margin-top:14px'>");
            foreach (var action in lc.ManualActionItems)
                sb.AppendLine($"<div class='rem'>⚠ {HtmlEncode(action)}</div>");
            sb.AppendLine("</div>");
        }
